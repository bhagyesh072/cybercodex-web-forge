...
const AnimatedModels = () => {
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      // Calculate mouse position relative to viewport for parallax effect
      setMousePosition({
    ...
    <section className="relative py-20 overflow-hidden bg-gradient-to-b from-cyber-blue-light/5 to-white">
      ...
      <div className="absolute inset-0 overflow-hidden">
        {[...Array(20)].map((_, i) => (
          <div 
            key={i}
            className="absolute bg-gradient-to-tr from-cyber-blue/10 to-cyber-blue-light/20 rounded-full"
            style={{
              width: `${Math.random() * 100 + 50}px`,
              height: `${Math.random() * 100 + 50}px`,
              top: `${Math.random() * 100}%`,
              left: `${Math.random() * 100}%`,
              transform: `translate(${mousePosition.x/3}px, ${mousePosition.y/3}px) scale(${1 + Math.sin(Date.now() / 1000 / (i + 1)) * 0.2})`,
              opacity: 0.4 + Math.sin(Date.now() / 1000 / (i + 1)) * 0.2,
              transition: 'transform 0.1s ease-out, opacity 4s ease-in-out',
      ...
      <div className="container mx-auto px-4 relative z-10">
        ...
        <div className="perspective-container">
          ...
          >
            {technologies.map((tech, idx) => (
              <div
                key={idx}
                className={`tech-card animate-on-scroll ${activeIcon === idx ? 'active' : ''}`}
                style={{ animationDelay: `${idx * 100}ms` }}
                onMouseEnter={() => setActiveIcon(idx)}
                onMouseLeave={() => setActiveIcon(null)}
              >
                <div 
                  className={`relative flex flex-col items-center p-6 rounded-xl bg-white border-2 
                              transition-all duration-300 hover:shadow-xl group
                              ${activeIcon === idx ? 'border-cyber-blue scale-105' : 'border-gray-100'}`}
                >
                  <div className="absolute -top-3 -right-3 w-16 h-16 bg-gradient-to-br from-cyber-blue-dark to-cyber-blue rounded-full opacity-0 group-hover:opacity-100 transition-opacity" />
                  <div className={`icon-wrapper relative z-10 mb-4 text-cyber-blue transition-transform duration-300 ${activeIcon === idx ? 'scale-110' : ''}`}>
                    {tech.icon}
                  </div>
                  <h3 className="text-xl font-bold mb-2 text-cyber-blue-dark">{tech.label}</h3>
                  <p className="text-gray-600 text-center">{tech.description}</p>
                  
                  <div className={`pulse-ring absolute inset-0 rounded-xl transition-all duration-300 ${activeIcon === idx ? 'opacity-100' : 'opacity-0'}`}>
        ...
        <div className="mt-20 relative">
          <div className="code-wave h-40 md:h-60 w-full overflow-hidden">
            {[...Array(20)].map((_, i) => (
              <div 
                key={i} 
                className="code-line absolute h-1 bg-gradient-to-r from-cyber-blue/20 via-cyber-blue to-cyber-blue/20 rounded-full" 
                style={{
                  width: `${Math.random() * 30 + 70}%`,
                  left: `${Math.random() * 20}%`,
                  top: `${(i / 20) * 100}%`,
                  animationDelay: `${i * 0.2}s`,
                  transform: `translateX(${Math.sin(Date.now() / 1000 + i) * 50}px)`,
                }}
              />
            ))}
          </div>
          
          <div className="cyber-orb absolute left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2">
